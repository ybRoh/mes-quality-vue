...
# ============================================================
# 발견사항 전체 목록 (static paths before /{cust_audit_id})
# ============================================================
...
# ============================================================
# 시정조치 전체 목록 (static paths before /{cust_audit_id})
# ============================================================
...
def list_all_actions(
    ...
    finding_map = {}
    if finding_ids:
        findings_q = db.query(QmsCustomerAuditFinding).filter(QmsCustomerAuditFinding.cust_finding_id.in_(finding_ids)).all()
        finding_map = {f.cust_finding_id: f.finding_no for f in findings_q}
...
# ============================================================
# 연간 요약 (static path before /{cust_audit_id})
# ============================================================
...
def customer_audit_summary(
    ...
    audits_q = db.query(QmsCustomerAudit).filter(
        extract("year", QmsCustomerAudit.audit_date) == audit_year
    ).all()

    total_audits = len(audits_q)
    pass_count = sum(1 for a in audits_q if a.result == "PASS")
    conditional_count = sum(1 for a in audits_q if a.result == "CONDITIONAL")
    fail_count = sum(1 for a in audits_q if a.result == "FAIL")

    audit_ids = [a.cust_audit_id for a in audits_q]
    findings_q = []
    if audit_ids:
        findings_q = db.query(QmsCustomerAuditFinding).filter(
            QmsCustomerAuditFinding.cust_audit_id.in_(audit_ids)
        ).all()

    total_findings = len(findings_q)
    major_nc = sum(1 for f in findings_q if f.finding_type == "MAJOR_NC")
    minor_nc = sum(1 for f in findings_q if f.finding_type == "MINOR_NC")
    observation = sum(1 for f in findings_q if f.finding_type == "OBSERVATION")

    finding_ids = [f.cust_finding_id for f in findings_q]
    actions_q = []
    if finding_ids:
        actions_q = db.query(QmsCustomerAuditAction).filter(
            QmsCustomerAuditAction.cust_finding_id.in_(finding_ids)
        ).all()

    total_actions = len(actions_q)
    completed_actions = sum(1 for a in actions_q if a.status in ("COMPLETED", "VERIFIED"))
    overdue_actions = sum(
        1 for a in actions_q
