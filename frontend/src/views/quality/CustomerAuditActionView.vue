<template>
  <div class="page-container">
    <PageHeader title="고객심사 시정조치" subtitle="고객심사 시정조치 관리">
      <template #actions>
    ...
    <el-tabs v-model="activeTab" type="border-card">
      ...
      <el-tab-pane label="조치상세" name="detail">
        ...
        <div v-else>
          ...
          <div class="action-detail-header">
            <div style="display: flex; align-items: center; gap: 12px; flex-wrap: wrap;">
              ...
              <el-tag :type="statusTagType(selectedAction.status)" effect="dark">
                {{ statusLabelText(selectedAction.status) }}
              </el-tag>
          ...
          <div class="detail-section">
            <div class="detail-label">고객 피드백</div>
            <div class="detail-content">{{ selectedAction.customer_feedback || '-' }}</div>
          </div>
        </div>
      </el-tab-pane>
    </el-tabs>
        ...
        <el-row :gutter="16" v-if="!isEditing">
          <el-col :span="12">
            <el-form-item label="조치 No." required>
              <el-input v-model="formData.action_no" placeholder="CCA-001" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="연계 발견사항 ID" required>
              <el-input-number
...
const formData = ref({
  cust_action_id: null as number | null,
...
function editAction(row: any) {
  ...
  formData.value = {
    cust_action_id: row.cust_action_id,
    finding_id: row.cust_finding_id || null,
...
async function deleteAction(row: any) {
  try {
    ...
    await customerAuditApi.deleteAction(row.cust_action_id)
    ElMessage.success('삭제되었습니다.')
    if (selectedAction.value?.cust_action_id === row.cust_action_id) {
...
async function submitAction() {
  try {
    ...
    if (formData.value.cust_action_id) {
      const res = await customerAuditApi.updateAction(formData.value.cust_action_id, payload)
      ElMessage.success('수정되었습니다.')
      // Update selected action if viewing it
      if (selectedAction.value?.cust_action_id === formData.value.cust_action_id) {
      ...
      payload.action_no = formData.value.action_no
      await customerAuditApi.createAction(formData.value.finding_id, payload)
...
async function verifyAction(row: any) {
  try {
    ...
    const res = await customerAuditApi.verifyAction(row.cust_action_id)
