<template>
  <div class="page-container">
    <PageHeader title="고객심사 발견사항" subtitle="고객심사 부적합/관찰사항 관리">
      <template #actions>
    ...
    <el-tabs v-model="activeTab" type="border-card">
      ...
      <el-tab-pane label="전체 발견사항" name="list">
        ...
        >
          ...
          <el-table-column prop="audit_no" label="연계 심사" width="140" />
          <el-table-column prop="finding_type" label="유형" width="100" align="center">
            <template #default="{ row }">
              <el-tag :type="findingTypeTag(row.finding_type)" size="small">
      ...
      <el-tab-pane label="상세" name="detail">
        ...
        <template v-if="selectedFinding">
          ...
          <div class="finding-detail-header">
            ...
            <el-tag :type="findingTypeTag(selectedFinding.finding_type)" effect="dark">
              {{ findingTypeLabel(selectedFinding.finding_type) }}
            </el-tag>
            <el-tag :type="statusTag(selectedFinding.status)">
          ...
          <div class="detail-section">
            ...
            <el-table :data="findingActions" border stripe v-loading="actionsLoading" size="small">
              ...
              <el-table-column prop="status" label="상태" width="100" align="center">
                <template #default="{ row }">
                  <el-tag :type="actionStatusTag(row.status)" size="small">
    ...
    >
      <el-form :model="findingForm" label-position="top">
        <el-row :gutter="16">
          <el-col :span="12">
            <el-form-item label="발견 No." required>
              <el-input v-model="findingForm.finding_no" placeholder="CF-001" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="연계 심사 ID" required>
              <el-input-number
                v-model="findingForm.cust_audit_id"
                :min="1"
    ...
    >
      <el-form :model="actionForm" label-position="top">
        <el-form-item label="조치 No." required>
          <el-input v-model="actionForm.action_no" placeholder="CCA-001" />
        </el-form-item>
...
const findingForm = ref({
  cust_finding_id: null as number | null,
...
function findingTypeTag(type: string): '' | 'success' | 'warning' | 'danger' | 'info' {
  switch (type) {
...
function actionStatusTag(status: string): '' | 'success' | 'warning' | 'danger' | 'info' {
  switch (status) {
...
async function loadFindings() {
  ...
  try {
    ...
    const res = await customerAuditApi.getFindings(params)
    findings.value = res.data.items || res.data
  } catch (e) {
    console.warn('고객심사 발견사항 목록 조회 실패:', e)
...
async function selectFinding(row: any) {
  try {
    const res = await customerAuditApi.getFinding(row.cust_finding_id)
...
async function loadFindingActions() {
  ...
  try {
    const res = await customerAuditApi.getFindingActions(selectedFinding.value.cust_finding_id)
...
function editFinding(row: any) {
  ...
  findingForm.value = {
    cust_finding_id: row.cust_finding_id,
    cust_audit_id: row.cust_audit_id || null,
...
async function deleteFinding(row: any) {
  try {
    ...
    await customerAuditApi.deleteFinding(row.cust_finding_id)
    ElMessage.success('삭제되었습니다.')
    if (selectedFinding.value?.cust_finding_id === row.cust_finding_id) {
...
async function submitFinding() {
  ...
  try {
    ...
    if (findingForm.value.cust_finding_id) {
      await customerAuditApi.updateFinding(findingForm.value.cust_finding_id, payload)
      ElMessage.success('수정되었습니다.')
    } else {
      if (!findingForm.value.cust_audit_id) {
        ElMessage.warning('연계 심사를 선택해야 합니다.')
        return
      }
      await customerAuditApi.createFinding(findingForm.value.cust_audit_id, payload)
...
async function submitAction() {
  ...
  try {
    ...
    await customerAuditApi.createAction(selectedFinding.value.cust_finding_id, payload)
